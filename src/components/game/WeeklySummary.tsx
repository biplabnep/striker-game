        ...
        <div className="relative px-5 py-4 border-b border-[#30363d] bg-[#161b22]">
          ...
          <div className="flex items-center justify-between relative">
            <div>
              ...
              <p className="text-xs text-[#8b949e]">Season {currentSeason} • Week {currentWeek - 1} → {currentWeek}</p>
            </div>
            <button onClick={onClose} className="p-1.5 rounded-lg bg-[#21262d] text-[#8b949e] hover:text-white transition-colors">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
              ...
              <div className={`rounded-lg p-4 ${
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-[#c9d1d9]">{latestMatch.homeClub.shortName || latestMatch.homeClub.name.slice(0, 3)}</span>
                  </div>
                  <div className="text-xl font-black text-white">
                    {latestMatch.homeScore} - {latestMatch.awayScore}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-[#c9d1d9]">{latestMatch.awayClub.shortName || latestMatch.awayClub.name.slice(0, 3)}</span>
                ...
                <div className="flex items-center justify-center gap-6 pt-2 border-t border-[#30363d]">
                  ...
                  <div className="text-center">
                    <div className="text-lg font-bold text-[#c9d1d9]">{latestMatch.playerMinutesPlayed}&apos;</div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-[#21262d] rounded-lg p-3 text-center">
                ...
                <p className="text-lg font-bold text-[#c9d1d9]">{player.form.toFixed(1)}</p>
                <p className="text-[10px] text-[#8b949e]">Form</p>
              </div>
              <div className="bg-[#21262d] rounded-lg p-3 text-center">
                <p className="text-lg font-bold text-amber-400">{player.morale}</p>
                <p className="text-[10px] text-[#8b949e]">Morale</p>
              </div>
              <div className="bg-[#21262d] rounded-lg p-3 text-center">
                <p className="text-lg font-bold text-blue-400">{player.fitness}</p>
                <p className="text-[10px] text-[#8b949e]">Fitness</p>
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#21262d] rounded-lg p-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#c9d1d9]">Sessions Available</span>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {gameState.activeEvents.slice(0, 3).map(event => (
                  <div key={event.id} className="bg-amber-900/15 border border-amber-800/20 rounded-lg p-3">
