  ...
  return (
    <Card className="bg-slate-900/80 backdrop-blur-sm border-slate-800/60 overflow-hidden">
      ...
      <CardContent className="px-4 pb-3 space-y-3">
        {/* Timeline Bar */}
        <div className="relative h-8 rounded-lg bg-slate-800/80 overflow-hidden">
          {/* Window regions */}
          {windows.map((w, i) => (
            <motion.div
              key={i}
            ...
            >
              ...
              <span className="absolute bottom-0.5 right-1 text-[7px] text-slate-500">
                Wk {w.start}-{w.end}
              </span>
            </motion.div>
          ))}
    ...
    >
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-[10px]">
          <span className="text-slate-400">Squad Quality</span>
        </div>
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-500 w-12 truncate">{currentClub.shortName}</span>
            ...
            <span className="text-[10px] text-slate-400 w-6 text-right">{currentClub.quality}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-500 w-12 truncate">{offerClub.shortName}</span>
            <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
            ...
            <span className="text-[10px] text-slate-400 w-6 text-right">{offerClub.quality}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-1 text-center">
        ...
        {/* League */}
        <div className="text-[9px] text-slate-600 text-left">League</div>
        <div className="text-[9px] text-slate-300">{formatLeague(currentClub.league)}</div>
        <div className="text-[9px] text-slate-300">{formatLeague(offerClub.league)}</div>

        {/* Reputation */}
        <div className="text-[9px] text-slate-600 text-left">Reputation</div>
        <div className="text-[9px] text-slate-300">{currentClub.reputation}</div>
        <div className={`text-[9px] font-medium ${offerClub.reputation > currentClub.reputation ? 'text-cyan-400' : offerClub.reputation < currentClub.reputation ? 'text-red-400' : 'text-slate-300'}`}>
          {offerClub.reputation > currentClub.reputation && <ChevronUp className="inline h-2.5 w-2.5" />}
          {offerClub.reputation}
        </div>

        {/* Squad Role */}
        <div className="text-[9px] text-slate-600 text-left">Role</div>
      ...
      <div className="space-y-1.5">
        <div className="text-[10px] text-slate-500">Playing Time Likelihood</div>
        <div className="flex items-center gap-2">
          <div className="flex-1 space-y-0.5">
            <div className="flex justify-between text-[9px]">
              <span className="text-emerald-500">{currentClub.shortName}</span>
              <span className="text-slate-500">{currentTime.label}</span>
            </div>
          ...
          <div className="flex-1 space-y-0.5">
            <div className="flex justify-between text-[9px]">
              ...
              <span className="text-slate-500">{offeredTime.label}</span>
            </div>
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
...
function AgentQualityCard({ agentQuality, reputation }: { agentQuality: number; reputation: number }) {
  ...
  const agentTier = useMemo(() => {
    ...
    if (agentQuality >= 20) return { label: 'Average Agent', color: 'text-slate-400', bg: 'bg-slate-500/10', border: 'border-slate-500/20', icon: Users };
      ...
      <CardContent className="px-4 pb-3 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <Badge className="bg-slate-800 text-[9px] text-slate-400 border-slate-700">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/50 rounded-lg p-2">
            <span className="text-[9px] text-slate-500 uppercase">Commission</span>
            <p className="text-xs font-semibold text-white">{commission}%</p>
          </div>
          <div className="bg-slate-800/50 rounded-lg p-2">
            <span className="text-[9px] text-slate-500 uppercase">Deal Quality</span>
            <p className="text-xs font-semibold text-slate-300">{dealQuality}</p>
          </div>
        </div>
            ...
            <AnimatePresence mode="wait">
              {!showUpgrade ? (
                <motion.button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700/50 text-[10px] text-slate-400 hover:text-emerald-400 hover:border-emerald-700/40 transition-colors"
                >
                ...
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="flex-1 h-7 border-slate-700 text-slate-400 text-[10px]"
        ...
        <Card className="bg-slate-900/80 backdrop-blur-sm border-slate-800/60 overflow-hidden">
          ...
          <CardContent className="px-4 pb-3">
            ...
            {player.contract.releaseClause && (
              <div className="mb-3 pt-2 border-t border-slate-800/60 flex items-center justify-between">
        ...
        <Card className="bg-slate-900/80 backdrop-blur-sm border-slate-800/60 overflow-hidden">
          <CardContent className="p-4">
            ...
            <div className="flex items-center justify-between mt-1">
              <span className="text-[9px] text-slate-600">Recent trend</span>
          ...
          <div className="space-y-3">
            <AnimatePresence>
              {transferOffers.map((offer, idx) => (
                ...
                >
                  <Card
                    className="bg-slate-900/80 backdrop-blur-sm border-slate-800/60 overflow-hidden relative"
                  >
                    ...
                    <CardContent className="p-4 relative z-10">
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        <div className="w-10 h-10 rounded-lg bg-slate-800/80 flex items-center justify-center text-xl border border-slate-700/50">
                          {offer.fromClub.logo}
                        </div>
                        <div className="flex-1">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[9px] text-slate-500">Transfer Fee</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                        ...
                        {offer.contractOffer.signingBonus ? (
                          <div className="bg-slate-800/50 rounded-lg p-2">
                            <span className="text-slate-500 text-[10px]">Signing Bonus</span>
                            <p className="font-semibold text-white text-xs">
                        ...
                        {offer.contractOffer.releaseClause ? (
                          <div className="bg-slate-800/50 rounded-lg p-2">
                            <span className="text-slate-500 text-[10px]">Release Clause</span>
            ...
            <AnimatePresence>
              {loanOffers.map((offer, idx) => (
                <motion.div
                  ...
                  transition={{ delay: idx * 0.06 }}
                >
                  <Card className="bg-slate-900/80 backdrop-blur-sm border-slate-800/60 overflow-hidden relative">
                    {/* Club gradient background overlay */}
                    <div
                      className="absolute inset-0 opacity-[0.04]"
                      style={{
                    ...
                    <CardContent className="p-4 relative z-10">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                        ...
                        <div className="bg-slate-800/50 rounded-lg p-2">
                          <span className="text-slate-500 text-[10px]">Wage Paid</span>
                      ...
                      <div className="mb-3 space-y-1">
                        <div className="flex items-center justify-between text-[10px]">
                          <span className="text-slate-500">Playing Time Estimate</span>
